...
**完全免费、美股交易时段每 30 分钟扫描、自动邮件通知的 QQQM 定投+补仓策略监控。**
...
以下数据由 GitHub Actions 在每次扫描时自动更新，反映最新市价和收益率：
...
| **汇率** | USD/HKD = 7.8 |
| **实时状态** | → 查看 [PORTFOLIO_STATUS.md](PORTFOLIO_STATUS.md)（美股交易时段每 30 分钟自动刷新） |
...
| **2022 熊市中** | 几乎每日触发，补仓 73 次 | 只在"上涨后回调"时触发，补仓 27 次 |
| **资金压力** | 极大（399,000 HKD） | 中等（261,000 HKD） |
| **回撤控制** | -20.32% | **-18.84%**（最佳，低 1.48pp） |
...
| **S3** (历史前高回撤) | 399,000 | 689,711 | **72.86%** | -20.32% | 60 | 73 |
| **S4** (上涨周期高点回撤) ⭐ | **261,000** | **440,881** | **68.92%** | **-18.84%** | 60 | **27** |
| **S5** (双条件补仓) | 399,000 | 689,711 | **72.86%** | -20.32% | 60 | 73 |

> **数据来源**：`qqqm_full_matrix_report.json`（2026-06-04 生成，14 天冷却期）。回测区间 2021-01-04 至 2025-12-30，共 1,254 个交易日，60 个月。
>
> ⭐ S4 以 **68.92%** 收益率位居第二，最大回撤控制最佳（-18.84%，比 S3 改善 1.48pp）。比 S3/S5 少投入 138,000 HKD（-35%），少补仓 46 次，收益率仅差 3.94 个百分点——资金效率优势显著。冷却期仅拦截 1 次触发，说明 S4 的「上涨周期高点回撤」条件本身已足够精准。
...
| 维度 | 最佳策略 | 数据 |
|------|---------|------|
| **收益率最高（14天）** | S3 / S5（前高点回撤） | 72.86% |
| **回撤控制最佳（14天）** | **S4**（上涨周期高点回撤）⭐ | -18.84% |
| **资金效率最优（14天）** | **S1**（纯定投） | 180K 投入 / 63.80% 收益 |
| **综合平衡度** | **S4** ⭐ | 收益亚军 + 回撤最低 + 省 35% 资金 |

> 💡 **S4 的核心优势**：在 2021-2025 年包含 2022 年大熊市的市场中，S4 比纯定投（S1）收益率高 5.12 个百分点（68.92% vs 63.80%），同时最大回撤更低（-18.84% vs -20.41%）。比 S3/S5 少投入 138,000 HKD（节省 35%），少补仓 46 次，收益率仅差 3.94 个百分点。冷却期设为 14 天时，5 年仅拦截了 1 次触发——说明 S4 的「上涨周期高点回撤」条件本身就筛选掉了低质量的补仓点。
...
**60 次定投 + 27 次补仓**，完整交易明细见 `qqqm_multi_strategy_report.md`（注：明细文件基于 30 天冷却期，共 23 次补仓；14 天冷却期为推荐配置，需要时可生成 14 天冷却期的完整交易明细）。
...
> 💡 可以看到补仓大多集中在 2022 年熊市底部区域（$108~$130 区间），2023-2025 的补仓则分布在每次上涨后的回调中。27 次补仓中有 12 次发生在 2022 年——这是 S4 超额收益的核心来源。

### 四策略买入点对比

![QQQM All Strategies Buy Points](docs/qqqm_buy_all.png)

> 📊 **从上到下 S2 → S3 → S4 → S5**：S3/S5 的补仓点明显更密集（73 次），遍布整个 5 年区间；S4 只在「上涨后回调」时触发，更聚焦于真正的回调机会。图表基于 14 天冷却期数据生成。
...
| 时间 | 说明 |
|------|------|
| **周一~周五，每 30 分钟** | GitHub Actions 自动扫描 QQQM 价格 |
| **EDT 夏令（3~11月）** | 13:30-20:00 UTC（香港 21:30-04:00） |
| **EST 冬令（11~3月）** | 14:30-21:00 UTC（香港 22:30-05:00） |
| **每天约 15 次有效扫描** | 边缘时段由脚本安全门自动跳过 |
| **触发补仓时** | 立即创建 GitHub Issue → 自动发邮件通知 |

> ⏰ **为什么是这个时间**：美股盘前/盘后流动性差、价格波动大，仅在正式交易时段扫描可避免假信号。每 30 分钟足够捕捉 5%+ 回撤事件。
...
**定投日：**
```
📅 日期: 2025-06-30  |  QQQM 收盘: $305.00
🔔 今日操作:
   → 【定投】投入 HKD 3,000 | 价格 $305.00
💰 总投入: HKD 9,000 | 📈 市值: HKD 10,046 | 💵 收益: +11.62%
```

**补仓日：**
```
📅 日期: 2025-07-05  |  QQQM 收盘: $289.75
🔔 今日操作:
...
| 审阅日期 | 审阅内容 | 结果 |
|---------|---------|------|
| 2025-06-03 | 五策略回测数据（QQQ 2020-2025，旧标的） | ✅ 已通过 |
| 2025-06-03 | S4 冷却期对比（QQQ，旧标的） | ✅ 已通过 |
...
| **2026-06-04** | **数据校正：S3/S5 对齐 full_matrix 14d 数据源，修正来源引用** | **✅ 已修正** |
...
| 文件 | 内容 | 备注 |
|------|------|------|
| `qqqm_multi_strategy_report.json` | QQQM 五策略回测结果（30 天冷却期） | 历史对比参考 |
| `qqqm_multi_strategy_report.md` | QQQM 完整报告含补仓明细（30 天冷却期） | 含每笔交易日期/价格 |
| `qqqm_full_matrix_report.json` | QQQM 全矩阵（S1-S5 × 0/7/14/30d 冷却期） | ⭐ 当前推荐配置数据源 |
| `qqqm_full_matrix_report.md` | 全矩阵可读报告（Markdown 格式） | 20 条结果 + 热力图数据 |
...
**部署完成后，你就拥有了一个美股交易时段每 30 分钟扫描的 S4 策略监控机器人，无论你在哪、电脑开不开机，触发补仓时都会准时邮件提醒你！**

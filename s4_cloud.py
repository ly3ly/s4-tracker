...
def update_portfolio_status(state, price_usd, date_str):
    """计算实时组合盈亏并写入 state.json + 生成 PORTFOLIO_STATUS.md"""
    rate = CONFIG["currency_rate"]
    shares = state.get("total_shares", 0)
    invested = state.get("total_invested", 0)

    if shares > 0 and invested > 0:
        value_hkd = shares * price_usd * rate
        profit_hkd = value_hkd - invested
        ret_pct = (profit_hkd / invested) * 100
        avg_cost = state.get("avg_cost_usd", invested / (shares * rate) if shares > 0 else 0)
    else:
        value_hkd = profit_hkd = ret_pct = 0.0
        avg_cost = 0.0
    ...
    state["portfolio"] = {
        ...
        "profit_hkd": round(profit_hkd, 2),
        "return_pct": round(ret_pct, 2),
        "avg_cost_usd": round(avg_cost, 2),
        "last_updated": date_str,
    }

    # 生成 PORTFOLIO_STATUS.md（供 README 引用）
    emoji = "🟢" if ret_pct >= 0 else "🔴"
    status_md = f"""<!-- AUTO-GENERATED by s4_cloud.py — DO NOT EDIT MANUALLY -->
## 📊 实时持仓状态

> 最后更新: {date_str} UTC | 每 30 分钟自动刷新（仅美股交易时段）

| 指标 | 数值 |
|------|------|
| **标的** | QQQM (Invesco NASDAQ 100 ETF) |
| **持有份额** | {shares:.4f} 股 |
| **当前价格** | ${price_usd:.2f} USD |
| **平均成本** | ${avg_cost:.2f} USD |
| **总投入** | HKD {invested:,.2f} |
| **持仓市值** | HKD {value_hkd:,.2f} |
| **累计收益** | {emoji} HKD {profit_hkd:+,.2f}（{ret_pct:+.2f}%） |

"""
    with open("PORTFOLIO_STATUS.md", "w", encoding="utf-8") as f:
        f.write(status_md)
    print(f"[PORTFOLIO] 市值 HKD {value_hkd:,.2f} | 收益 {ret_pct:+.2f}% | → PORTFOLIO_STATUS.md")
...
def main():
    ...
    # 更新组合盈亏 → state.json + PORTFOLIO_STATUS.md
    update_portfolio_status(state, price_data["close"], date_str)
    save_state(state)  # 二次保存（含 portfolio 字段）
